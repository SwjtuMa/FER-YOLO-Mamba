    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.mlp_branch:
            ...
            self.mlp = _MLP(in_features=hidden_dim//2, hidden_features=hidden_dim//2, act_layer=mlp_act_layer, drop=mlp_drop_rate, channels_first=channel_first)

    def _forward(self, input: torch.Tensor):
        if self.ssm_branch:
            if self.post_norm:
                x = self.drop_path(self.norm(self.op(input)))
            else:
                x = self.drop_path(self.op(self.norm(input)))
        if self.mlp_branch:
            if self.post_norm:
...
class YOLOPAFPN(nn.Module):
    ...
    def forward(self, input):
        ...
        print("P5_upsample",P5_upsample.shape)
        ...
        print("P4_upsample",P4_upsample.shape)
        
        for blk3 in self.C3_p3:
            P3_out = blk3(P4_upsample)       # torch.Size([8, 40, 40, 128])
    #    P3_out=P3_out.permute(0,3,1,2) # 

        P3_out1=P3_out.permute(0,3,1,2) # 
        P3_out=self.reduce_conv111(P3_out1)
        P3_downsample   = self.bu_conv2(P3_out1)
        
      #  P3_downsample   = self.bu_conv2(P3_out)   #  torch.Size([8, 128, 20, 20])
        #-------------------------------------------#
